		...
		<!--  Original Target XPath:  ref:citations/ref:cite4thisresource   -->
	            ...
	            <ref:citations>
	                <xsl:choose>
	                    ...
	                    <xsl:otherwise>
	                        <!-- Awantika:2017-11-28-lnci:cite attribute id was getting created twice one for ref:parallelcite and one for ref:cite4thisresource.
	                        Webstar# 7051426-->
	                        <xsl:if test="child::ci:cite[@type='cite4thisdoc']">
	                        <ref:cite4thisresource>
	                            <xsl:attribute name="citetype">
	                                <xsl:text>reporter</xsl:text>
	                            </xsl:attribute>
	                ...
	                <xsl:apply-templates select="following-sibling::case:reportercite[ci:cite[@type='cite4thisdoc']]" mode="secondreporter"/>
...
<!--        <xsl:apply-templates select="following-sibling::case:courtinfo/case:filenum"/>-->
    ...
    <xsl:template match="case:reportercite[ci:cite[@type='cite4thisdoc']]" mode="secondreporter">
        <xsl:choose>
            <xsl:when test="count(ci:cite) &gt; 1">
                <xsl:for-each select="ci:cite">
                    <ref:cite4thisresource>
    ...
    <xsl:template match="case:reportercite[ci:cite[not(@type='cite4thisdoc')]]" mode="wrapinfo">
        <xsl:choose>
            <xsl:when test="count(ci:cite) &gt; 1">
                <xsl:for-each select="ci:cite">
                    <ref:parallelcite citetype="parallel">

...
<!-- Sudhanshu Srivastava edited on 15-May-2017-->
	...
	<xsl:template match="case:hearingdates">
		<!--  Original Target XPath:  /casedigest:casedigest/casedigest:head/casedigest:caseinfo/decision:dates/decision:decisiondate   -->
	        <decision:dates>
	            <xsl:choose>
	                <xsl:when test="date[@day and @month and @year]">
	                    <decision:argueddate day="{date/@day}" month="{date/@month}" year="{date/@year}">
	                        <date-text>
	                            <xsl:value-of select="concat(key('monthgrep',date/@month,$monthmapping)/@value,' ',date/@day,', ',date/@year)"/>
	                        </date-text>
	                    </decision:argueddate>
	                </xsl:when>
	                <xsl:otherwise>

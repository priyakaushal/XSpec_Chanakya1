...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:dita="http://dita.oasis-open.org/architecture/2005/" 
  ...
  <title><sourcexml>level</sourcexml> becomes <targetxml>form:form</targetxml> <lnpid>id-NZ09-24221</lnpid></title>
  <body>
    <section>
      <p>If <sourcexml>level</sourcexml> comes with following <sourcexml>@leveltype</sourcexml> values:
        ...
        <b>or</b> <sourcexml>level[@lelvetype="comm.chap"]</sourcexml> occurs as direct parent of any listed above <sourcexml>level[@leveltype]</sourcexml> values and no another <sourcexml>level[@leveltype]</sourcexml> value encountered then <sourcexml>level</sourcexml> becomes <targetxml>form:form</targetxml> with following attributes:
        <ul>
	...
	<!--    Original DITA file location:  DITA\ConversionInstructions\Rosetta\DITA-PAC\NZ09_Comm_Form_Prec_Text\Comm-Form-Pre_level2.dita  -->
	...
	<xsl:template match="level[@leveltype=$formLeveltypes] | 
		level[@leveltype='comm.chap'][level[@leveltype=$formLeveltypes]][not(level[@leveltype!=$formLeveltypes])]" priority="25">
		<form:form>
	...
	<xsl:template match="heading[parent::bodytext]" mode="form">
		<form:h>
			<xsl:copy-of select="@* except (@typesize)"/>

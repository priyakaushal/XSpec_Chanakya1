...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:dita="http://dita.oasis-open.org/architecture/2005/" xmlns:source_cttr="http://www.lexis-nexis.com/glp/cttr" xmlns:classify="http://www.lexisnexis.com/xmlschemas/content/shared/classification/1/" xmlns:cttr="urn:x-lexisnexis:content:citator:sharedservices:1" version="2.0" exclude-result-prefixes="dita source_cttr">
   ...
   <title>catchwordgrp <lnpid>id-HK08-37609</lnpid></title>
   <body>
      <section>
...
&lt;/cttr:descriptors&gt;
	...
	<xsl:template match="catchwordgrp[not(preceding-sibling::catchwordgrp)]"><!--  TODO:  correct match pattern to prepend "source_" in front of appropriate ns prefixes!!  -->

		<!--  Original Target XPath:  cttr:descriptors/cttr:descriptorsection@descriptortype="catchwords-and-digest"   -->
			<cttr:descriptorsection>
			  <xsl:attribute name="descriptortype">catchwords-and-digest</xsl:attribute>
				<xsl:apply-templates select="@* | node()"/>
			  <xsl:for-each select="following-sibling::catchwordgrp">
			    <xsl:apply-templates select="node()"/>
			  </xsl:for-each>
			  <xsl:apply-templates select="following-sibling::source_cttr:summary"/>
			</cttr:descriptorsection>
		
	</xsl:template>

  <xsl:template match="catchwordgrp[preceding-sibling::catchwordgrp]"/>

...
&lt;/figure&gt;
  ...
  <xsl:template match="figure/p[preceding-sibling::*[self::caption]][not(text/inlineobject or text/link)][$streamID='UK08OR']" priority="21"/>
	...
	<xsl:template match="figure/p" priority="20">
	    <xsl:for-each-group select="*" group-adjacent="if (self::heading or self::inlineobject or self::link 
	        or self::text[not(child::*[not(self::heading or self::inlineobject or self::link)])]) then 0 else 1">
    ...
    <xsl:template match="figure/p/text" priority="20">
        <xsl:choose>
            <xsl:when test="parent::p[child::*[not(self::heading or self::inlineobject or self::link or     

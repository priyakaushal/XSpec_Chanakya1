...
<xsl:stylesheet xmlns="http://www.lexisnexis.com/xmlschemas/content/shared/base/1/"
                ...
                <ul>
                    <li class="- topic/li "> The conversion of
                        ...
                        scenarios: <ul>
                            ...
                            <li>If text of <sourcexml>leg:level-not-in-force</sourcexml> starts with
                                normal text <b>"NOTE:"</b> OR <b>"Note:"</b> but it is an official
                                note that is added by the government then convert it to
    ...
    <xsl:template match="leg:level-not-in-force">
        ...
        <xsl:choose>
            ...
            <xsl:when test="leg:level-vrnt/leg:levelbody/leg:bodytext[child::l[child::li[child::p[child::text[starts-with(., 'Editor''s note:') or starts-with(., 'Note de l''éditeur:') or starts-with(., '[Editor’s note:') or starts-with(., '[Note de l')]]]]]">
                <xsl:apply-templates select="descendant::leg:bodytext | descendant::leg:marginal-note"/>
            </xsl:when>
            <xsl:when
                ...
                <primlaw:level leveltype="amendment">
                    ...
                    <primlaw:bodytext>
                        <note notesource="official" notetype="other">
                            <bodytext>
                                <p>
                                    <xsl:apply-templates select="descendant::text[not(starts-with(., 'Editor''s note:') or starts-with(., 'Note de l''éditeur:') or starts-with(., '[Editor’s note:') or starts-with(., '[Note de l'))][following-sibling::*[1][self::text[starts-with(., 'Note:') or starts-with(., 'NOTE:')]]]"/>
                                    <xsl:apply-templates

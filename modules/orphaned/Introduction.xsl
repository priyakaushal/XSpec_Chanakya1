...
&lt;/casedigest:casedigest&gt;
  ...
  <xsl:variable name="outputSchemaLoc" select="'http://www.lexisnexis.com/xmlschemas/content/legal/case-digest/1/sch_casedigest.xsd'"/>
  ...
  <xsl:template match="CASEDOC">    
		...
		<casedigest:casedigest>
			<xsl:for-each select="in-scope-prefixes($lexisAdvNamespaces)">
				<xsl:namespace name="{ . }"
					select="namespace-uri-for-prefix(., $lexisAdvNamespaces)"/>
			</xsl:for-each>
